...
サーバーの状態だけでなく、ゲーム自体の攻略にも答える。**参照先は paldb.cc ドメインのみに限定してある。** 制限はドメイン単位でパスは絞っていないが、日本語の攻略ページ（`/ja`）を使うよう指示してある。日本語の攻略サイトは更新が止まった広告過多のものが大半で、検索を開放すると質が落ちるため。paldb.cc が扱わない話題には「見つからなかった」と答える。
